...
- `run-recording-prep.ts` — `runRecordingPrep(dealId)`:
  ...
  - **Split audit (part 1)** `recording.evaluated` (`metadata { venue, count }`) — before any write.
  - **Per cover sheet** (co-transactional): `persistCoverSheet`.
  - **Split audit (part 2)** `recording.prepared` (`metadata { venue, count }`) — after the cover-sheet
    writes. This is the **terminal for the dormant + live-pending path** (cover sheets persisted;
    submission dormant or awaiting the authority). Honest naming: the Fixture transmits nothing, so
    "prepared" — not "submitted" — is what durably happened.
  - `const submission = await recordingAdapter.submit(plan);` then, **only if
    `submission.submitted && submission.submissionId`** (false for the dormant Fixture — so the
    branches below are live/test-only), `poll(submissionId)` once (single-pass) and branch on
    `status`:
    - `accepted` → `persistRecordingCoordinates` + audit `recording.completed` (`metadata { venue }`).
      The agent does **not** flip the deal to `completed` (a processor/`transitionDealStatus` action —
      hard rule #6 stays a human-gated step; avoids agent re-entrancy into the status write path).
    - `rejected` → audit `recording.rejected` (`metadata { venue, reason }` — a static token). The
      durable Exception-Triage signal.
    - `pending` / `not_submitted` → no extra audit (`recording.prepared` already covers it).
...
Add labels for the four new audit actions (`recording.evaluated`, `recording.prepared`,
...
- **App** (`apps/web`):
  - `run-recording-prep.test.ts` — happy path (NYC refi): persists 1 cover sheet + `recording.evaluated`
    → `recording.prepared`; upstate refi enqueues the `county_cover_sheet`; idempotent: skips when a
