...
- **Next step:** M10 is closed; **M11 is not yet planned.** The highest-value follow-ups from ADR 0010 (do before/alongside M11): wire **OTel spans** on `runIntake` + the Server Action (CLAUDE.md §8 mandates every agent trace — not yet done); the **AI-Gateway adoption slice** (route the intake narrative + `@cema/search`'s classifier through the Gateway per spec §4 — both call the provider directly today); the **WDK wrap** once `@vercel/workflow` is installed. **Gating prerequisites for Connor (now blocking agent validation):** (1) provision `TYPESENSE_API_KEY`, `TYPESENSE_HOST`, `MEM0_API_KEY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `CRON_SECRET` — plus `BRAINTRUST_API_KEY` + `ANTHROPIC_API_KEY` to actually run the intake eval + narrative (runbook: `docs/runbooks/m7-env-var-provisioning.md`); (2) secure a design partner (spec §13.1 — overdue, on the critical path to validate any agent end-to-end); (3) confirm the NY recording-tax rate table the Intake Agent's savings math depends on (plan §6 / ADR 0010 carry-over #4).
...
- **Phase 1 Month 10 carry-overs to M11+ (9 items — see ADR 0010 for detail):**
  1. **OTel traces on the agent:** add spans at the `runIntake` await boundaries + the Server Action. CLAUDE.md §8 mandates every agent trace; the injected-deps seam isolates the natural span points. **Not yet wired.**
...
- **Code:** 19 workspace packages (added `@cema/agents-intake` in M10) + 1 Next.js 16 app. Tests: ~556 passing across 22 packages (apps/web: 248 passed + 2 skipped / 57 files unchanged; package tests sum: 308 across 44 files — M9's 232 + the new intake package's 76 across 6 files) + 1 Neon-gated intake-agent RLS integration test + 1 Playwright e2e (label-gated). 31 migrations on Neon dev branch (0000–0030, no new migrations in M8, M9, or M10 — M10 reuses the existing `deals`/`existing_loans`/`audit_events` schema). Vercel production + preview deploys both live; CodeRabbit reviewing every PR.
