...
- **Next step:** **M10, M11, M12, and M13 (both phases) are closed.** M13 Phase 1 shipped the **Collateral IDP Agent** (`@cema/agents-collateral-idp`, the third Layer 3 agent) as PR #88 (ADR 0015), and Phase 2 shipped its companion the **Chain-of-Title Agent** (`@cema/agents-chain-of-title`, the fourth Layer 3 agent) as PRs #91–#95 (ADR 0016) — detail in the Phase line above. **The M14 milestone scope awaits Connor's direction.** The strongest available slices — which turn the four shipped agents into an end-to-end flow — are: **wire the triggers** so the agents actually run (a document-upload/`deal_status` hook → IDP → a post-IDP hook → Chain-of-Title; `collateral_chase` → Outreach; Chain-of-Title's `re_chase` → hand off to the Outreach Agent — **nothing invokes any agent today**); the **real integration adapters** (the Encompass `LosAdapter`, ADR 0010 carry-over #5; the Resend `ServicerChannelAdapter`, ADR 0014 carry-over #1; the OCR/IDP vendor adapter, ADR 0015 carry-over #1); a **deal-scoped attorney/processor review surface** that renders IDP gate-required instruments + Chain-of-Title re-chase/attorney-review findings (ADR 0015 carry-over #4 + ADR 0016 carry-over #1); and **promoting `InstrumentRecord` to a shared `@cema/collateral` package** (today type-only re-exported from the IDP package — ADR 0015 carry-over #5 / ADR 0016). (**OTel tracing is wired** for the Intake Agent — `intake.run` + three child spans + the `intake.run_from_los` Server Action span, PII-safe attributes — per ADR 0011; carry-over #1 resolved.) **Gating prerequisites for Connor (still blocking end-to-end agent validation):** (1) provision `TYPESENSE_API_KEY`, `TYPESENSE_HOST`, `MEM0_API_KEY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `CRON_SECRET` — plus `BRAINTRUST_API_KEY`, `ANTHROPIC_API_KEY`, and `AI_GATEWAY_API_KEY` (the narrative gate flipped to the Gateway key in PR-A) to actually run the intake eval + narrative (runbook: `docs/runbooks/m7-env-var-provisioning.md`) — plus `RESEND_API_KEY` + a verified sending domain to activate the outreach channel (ADR 0014 carry-over #2); (2) secure a design partner (spec §13.1 — overdue, on the critical path to validate any agent end-to-end); (3) confirm the NY recording-tax rate table the Intake Agent's savings math depends on (plan §6 / ADR 0010 carry-over #4).
...
- **Phase 1 Month 13 Phase 2 (Chain-of-Title) carry-overs to M14+ (7 items — see ADR 0016 for detail):**
  1. **Real route actuators:** `routeReChase` / `openAttorneyReview` are dormant no-ops — routing is durable solely via the `chain.routed` audit event (counts in metadata) + the in-memory `RouteDecision[]` returned to the caller. Wire the real re-chase trigger (hand off to the Servicer Outreach Agent) + a review surface that renders re-chase vs. attorney-review items, each dispatched idempotently keyed `chain:<dealId>:break:<hash>`. Until then findings are audited but not rendered or acted on.
  2. **Wire a trigger:** nothing invokes `runChainOfTitleFromDeal` yet; the natural trigger is "IDP finished persisting instruments for a deal" (a post-IDP hook or `deal_status` transition). M14 owns this.
...
- **Phase 1 Month 13 carry-overs to M14+ (10 items — see ADR 0015 for detail):**
  ...
  2. **Wire a trigger:** a document-upload or `deal_status`-change hook (or cron) that calls `runCollateralIdpFromDeal` (or the durable variant). Nothing invokes the agent today.
