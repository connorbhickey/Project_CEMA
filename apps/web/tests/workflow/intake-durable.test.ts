 ...
 *
 * DEFERRED — gated off by default (ADR 0013 carry-over #5). The @workflow/vitest
 * builder externalizes workspace packages, and our @cema/* packages are raw TS
 * source (no dist/), so the in-process Local World cannot load the step bundle
 * under Node's ESM loader. Meanwhile the orchestration unit test
 * (intake.workflow.test.ts, mocked steps) is the authoritative behavioral guard;
 * this suite stands as the executable spec for the real start() → run.returnValue
 * proof, runnable once the toolchain bundles workspace TS-source. Opt in with
 * RUN_WDK_INTEGRATION=1 (plus DATABASE_URL). See the skip gate below for detail.
 */
...
//      externalizes workspace packages, but our @cema/* packages are published as
//      raw TS source (no dist/ — consumed everywhere via Turbopack
//      `transpilePackages`). So the in-process Local World loads the step bundle
//      through Node's raw ESM loader, which rejects @cema/agents-intake's
//      extensionless re-exports (ERR_MODULE_NOT_FOUND for './types'). Production is
//      unaffected: `withWorkflow` compiles the steps via Turbopack, which honors
//      transpilePackages. Until the WDK builder can bundle workspace TS-source (or
//      @cema/agents-intake ships a build), this stays off so `pnpm test:workflow`
//      is green. See ADR 0013 carry-over #5 for the exact resolution paths.
