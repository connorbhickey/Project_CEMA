...
describe('getEmail', () => {
  ...
  it('returns communication + emailThread on happy path', async () => {
    const rows = [
      {
        communications: { id: COMM_ID, kind: 'email', dealId: DEAL_ID },
        email_threads: { id: 'thread-1', subject: 'Test', communicationId: COMM_ID },
      },
    ];
    ...
    expect(result?.emailThread?.subject).toBe('Test');
  });

  it('returns null emailThread when left-join finds none', async () => {
    const rows = [
      {
        communications: { id: COMM_ID, kind: 'email', dealId: DEAL_ID },
        email_threads: null,
    ...
    const result = await getEmail(DEAL_ID, COMM_ID);
    expect(result?.emailThread).toBeNull();
  });
});
