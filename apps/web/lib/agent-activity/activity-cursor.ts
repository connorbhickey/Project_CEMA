...
// Canonical UUID shape (8-4-4-4-12 hex). Deliberately NOT the strict RFC-4122
// v4 pattern (`[1-5]` version / `[89ab]` variant): Postgres's `uuid` type — and
// `defaultRandom()` — accept any canonical hex uuid, including the all-zeros
// fixtures the integration tests use, so a stricter regex would reject valid
// ids. The point is only to reject non-uuid junk before it reaches the column.
 ...
 *  first page, never throws). Splits on the FIRST `_` (an ISO-8601 timestamp
 *  contains none), then validates the id is a canonical UUID: the id flows into
 *  `lt(auditEvents.id, cursor.id)` against a `uuid` column, so a non-uuid id
 *  must degrade to null here rather than reach the DB and throw an
...
export function parseActivityCursor(raw: string | undefined | null): ActivityCursor | null {
  ...
  if (!CANONICAL_UUID.test(id)) return null;
